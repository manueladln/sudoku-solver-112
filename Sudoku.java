...
public class Sudoku {    
    ...
    /*** ADD ANY ADDITIONAL METHODS HERE. ***/
     private boolean isSafeToPlace(int n, int val){
        int col = n%9;
        int row = n/9;
        boolean valinsubgrid = this.subgridHasVal[row/3][col/3][val];
        boolean valinrow = this.rowHasVal[row][val];
        boolean valincol = this.columnHasVal[col][val];
        if (!valinsubgrid && !valinrow && !valincol){
             return true;
         }
        return false;
